...
import { updateStrategy, getStrategyDetails } from "../routes/apiRoutes"; // Assuming you have an updateStrategy function
import styles from "../pages/CreateStretegy.module.css";
...
function mapToFields(condition) {
  return {
    left_multiplier: condition.left_multiplier,
    left_ohlc: condition.left_ohlc,
    left_offset: condition.left_candle_offset,
    operator: condition.operator,
    right_multiplier: condition.right_multiplier,
    right_ohlc: condition.right_ohlc,
    right_offset: condition.right_candle_offset,
    right_indicator: condition.right_indicator,
    right_factor: condition.right_factor,
    right_candle_count: condition.right_candle_count,
    left_indicator: condition.left_indicator,
    left_factor: condition.left_factor,
    left_candle_count: condition.left_candle_count,
    constant_value: condition.constant_value,
  };
}
...
function PreConditionFields({ type, value, onSave, onCancel, onClear }) {
...
export default function EditStrategiesModal({ open, strategy, onClose, onSave }) {
  const [strategyName, setStrategyName] = useState("");
  const [description, setDescription] = useState("");
  const [buyPreGroups, setBuyPreGroups] = useState([]);
  const [buyLogic, setBuyLogic] = useState([]);
  ...
  const [buyConditionType, setBuyConditionType] = useState("");
  ...
  useEffect(() => {
    console.log("Edit modal useEffect triggered. Open:", open, "Strategy:", strategy);
    const fetchDetails = async () => {
      if (!strategy?.id) {
        console.log("Strategy ID is missing, skipping API call.");
        return;
      }

      console.log("Fetching details for strategy ID:", strategy.id);
      setLoading(true);
      setError("");
      try {
        const response = await getStrategyDetails(strategy.id);
        console.log("API response:", response);
        const strategyData = response.data;

        if (strategyData && Array.isArray(strategyData.conditions)) {
          console.log("Populating form with data:", strategyData);
          setStrategyName(strategyData.name || "");
          setDescription(strategyData.description || "");

          const buyConditions = strategyData.conditions.filter(c => c.type === 'pre_buy');
          const sellConditions = strategyData.conditions.filter(c => c.type === 'pre_sell');
          const buyMainCondition = strategyData.conditions.find(c => c.type === 'buy');
          const sellMainCondition = strategyData.conditions.find(c => c.type === 'sell');
          const targetCondition = strategyData.conditions.find(c => c.type === 'target');
          const stopLossCondition = strategyData.conditions.find(c => c.type === 'stop_loss');

          if (buyConditions.length > 0) {
            setBuyPreGroups(buyConditions.map((c, i) => ({
              id: i,
              type: c.comparison_type,
              fields: mapToFields(c),
              showFields: false,
              summary: getSummary(c.comparison_type, mapToFields(c))
            })));
            setBuyLogic(buyConditions.slice(0, -1).map(c => c.logic_operator || 'AND'));
          } else {
            setBuyPreGroups([{ id: 1, type: "", fields: null, showFields: false, summary: null }]);
            setBuyLogic([]);
          }

          if (sellConditions.length > 0) {
            setSellPreGroups(sellConditions.map((c, i) => ({
              id: i,
              type: c.comparison_type,
              fields: mapToFields(c),
              showFields: false,
            ...
            setSellLogic(sellConditions.slice(0, -1).map(c => c.logic_operator || 'AND'));
          } else {
            setSellPreGroups([{ id: 1, type: "", fields: null, showFields: false, summary: null }]);
            setSellLogic([]);
          }

          if (buyMainCondition) {
            setBuyConditionType(buyMainCondition.comparison_type);
            if (buyMainCondition.comparison_type === 'ohlc_vs_ltp') {
              setBuyOhlcLtpFields(buyMainCondition);
              setBuyConditionSummary(
                `${buyMainCondition.left_multiplier || 1} ${
                  buyMainCondition.left_ohlc || "OHLC"
                } ${buyMainCondition.left_candle_offset || 1} ${
                  buyMainCondition.operator || ">"}`
              )
            }
          }

          if (sellMainCondition) {
            setSellConditionType(sellMainCondition.comparison_type);
            if (sellMainCondition.comparison_type === 'ohlc_vs_ltp') {
              setSellOhlcLtpFields(sellMainCondition);
              setSellConditionSummary(
                `${sellMainCondition.left_multiplier || 1} ${
                  sellMainCondition.left_ohlc || "OHLC"
                } ${sellMainCondition.left_candle_offset || 1} ${
                  sellMainCondition.operator || ">"}`
              )
            }
          }

          if (targetCondition) {
            setTarget(targetCondition.sl_tp_value);
            setTargetType(targetCondition.value_type === 'percent' ? 'Percent' : 'Points');
          }

          if (stopLossCondition) {
            setStopLoss(stopLossCondition.sl_tp_value);
            setStopLossType(stopLossCondition.value_type === 'percent' ? 'Percent' : 'Points');
          }
        } else {
          console.error("Invalid data received for strategy:", strategyData);
          throw new Error("Invalid data received for strategy.");
        }
      } catch (err) {
  ...
  // New: Clear handler for Buy
  const handleBuyClear = (idx) => {
    setBuyPreGroups((groups) =>
      groups.map((g, i) =>
        i === idx
          ? { ...g, type: "", fields: null, summary: null, showFields: false }
          : g
      )
    );
    setModalOpen(false);
  ...
  // New: Clear handler for Sell
  const handleSellClear = (idx) => {
    setSellPreGroups((groups) =>
      groups.map((g, i) =>
        i === idx
          ? { ...g, type: "", fields: null, summary: null, showFields: false }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await updateStrategy(strategy.id, payload);
  ...
  return (
    <Modal isOpen={open} onClose={onClose}>
     ...
     <Toast
      ...
      />
      <div style={{ height: '80dvh', overflowY: 'auto', padding: '20px', boxSizing: 'border-box' }}>
        <div className={styles.header}>Edit Strategy</div>
            ...
            <div className={styles.card}>
              ...
              {buyPreGroups.map((group, idx) => (
                <PreConditionGroup
                  ...
                  onLogicClick={(logic) =>
                    handleBuyLogicClick(idx - 1, logic)
                  }
            ...
            <div className={styles.card}>
              ...
              {sellPreGroups.map((group, idx) => (
                <PreConditionGroup
                  ...
                  onLogicClick={(logic) =>
                    handleSellLogicClick(idx - 1, logic)
                  }
                  showFields={false} // Always false, handled by modal
                  summary={group.summary}
                />
        ...
        <div className={styles.targetStopWrapper}>
          ...
          <div className={styles.targetStopCard}>
            <div
              className={styles.targetStopLabel}
              style={{ minWidth: 80 }}
            >
              Stop Loss
            </div>
        ...
        <div className={styles.actionBtns}>
          ...
          <button className={styles.paperBtn} onClick={onClose}>
            Cancel
          </button>
        </div>
      </div>
      ...
      <Modal
        isOpen={modalOpen}
        onClose={() => {
          setModalOpen(false); // Only close modal, do not reset anything
        }}
      >
        {modalConditionType && (
          <div
          ...
          >
            ...
            <PreConditionFields
              ...
              onClear={() => {
                if (modalType === "buy") handleBuyClear(modalIdx);
                else handleSellClear(modalIdx);
              }}
            />
          </div>
        )}
      ...
      <Modal
        ...
        onClose={() => {
          setBuyOhlcLtpModalOpen(false); // Only close modal, do not reset type/summary
        }}
      >
      ...
      <Modal
        ...
        onClose={() => {
          setSellOhlcLtpModalOpen(false); // Only close modal, do not reset type/summary
        }}
      >
        <div style={{ minWidth: 400 }}>
          <div
