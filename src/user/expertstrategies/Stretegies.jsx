...
export function Stretegies() {
  ...
  const [noActiveStrategies, setNoActiveStrategies] = useState(false);
  ...
  const fetchStrategies = async (pageNum = page, size = pageSize) => {
    ...
    setLoading(true);
    setError("");
    setNoActiveStrategies(false);
    console.log("Fetching strategies with params:", {
      page: pageNum,
      page_size: size,
    });
    try {
      ...
      const isSuccessResponse = res?.data?.success === true;
      const arr = Array.isArray(res?.data?.data) ? res.data.data : [];

      if (isSuccessResponse && arr.length === 0) {
        // Successful response but no active strategies
        setStrategies([]);
        setNoActiveStrategies(true);
        const meta = res.meta || {};
        setTotalItems(
        ...
        console.log("Success response with no active strategies");
      } else if (arr.length > 0) {
        setStrategies(arr);
        const meta = res.meta || {};
        setTotalItems(
          typeof meta.total_items === "number" ? meta.total_items : arr.length,
        );
        setTotalPages(
          typeof meta.total_pages === "number" ? meta.total_pages : 1,
        );
        ...
        // Error case - API failed or returned error
        setStrategies([]);
          ...
          <>
            ...
            >
              ...
              >
                ...
                <span
                  style={{ fontWeight: 600, fontSize: 16, margin: "0 8px" }}
                >
                  {page}
                </span>
